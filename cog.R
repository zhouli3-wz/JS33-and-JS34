cog <- scan("cog.class.annot.xls",what="character",sep="\n")
...
pdf(file="cog.pdf",10,7)
#plot.new()
def.par <- par()
layout(matrix(c(1,2),nr=1),widths=c(20,12))
library(RColorBrewer)
mycolors<-c("#0099FF","#85FF33","#FF335C","#64FAE6",brewer.pal(7, "Set2"),brewer.pal(6, "Set3"),brewer.pal(8, "Pastel1"))
op1 <- par(mar=c(4.1,5.1,4.1,0))
a <- barplot(cog.num,col=mycolors,ylim=c(0,max(cog.num)*1.1),cex.names=0.8,las=1,
axis.lty=0,cex.axis=0.6,xaxt="n"
)
box()
mtext("Function Class",side=1,line=2,font=2,cex=0.8)
mtext("Number of genes",side=2,line=2.5,font=2,cex=0.8)
par(cex.axis=0.6)
...
par(op1)
par(def.par)
title("COG Function Classification of 2023_JSZJ_023.protein.fa Sequence")
